...
local loadPlayer = "SELECT `accounts`, `job`, `job2`, `job_grade`, `job_grade2`, `group`, `position`, `inventory`, `skin`, `loadout`, `metadata`"
...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    local job2, grade2 = result.job, tostring(result.job_grade2)
...
AddEventHandler("esx:playerLoaded", function(_, xPlayer)
    ...
    local jobKey2 = ("%s:count"):format(job2)

    ESX.JobsPlayerCount[job2] = (ESX.JobsPlayerCount[job2] or 0) + 1
    GlobalState[jobKey2] = ESX.JobsPlayerCount[job2]
end)
...
AddEventHandler("esx:setJob2", function(_, job2, lastJob2)
    local lastJobKey = ("%s:count"):format(lastJob2.name)
    local jobKey = ("%s:count"):format(job2.name)
    local currentLastJob = ESX.JobsPlayerCount[lastJob2.name]

    ESX.JobsPlayerCount[lastJob2.name] = ((currentLastJob and currentLastJob > 0) and currentLastJob or 1) - 1
    ESX.JobsPlayerCount[job2.name] = (ESX.JobsPlayerCount[job2.name] or 0) + 1

    GlobalState[lastJobKey] = ESX.JobsPlayerCount[lastJob2.name]
    GlobalState[jobKey] = ESX.JobsPlayerCount[job2.name]
end)
