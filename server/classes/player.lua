...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, job2, loadout, name, coords, metadata)
	...
	_ExecuteCommand(('add_principal player.%s group.%s'):format(self.source, self.job.name))
	_ExecuteCommand(('add_principal player.%s group.%s'):format(self.source, self.job2.name))
    ...
    function self.setJob2(newJob, grade)
        ...
        local jobObject2, gradeObject2 = ESX.Jobs[newJob], ESX.Jobs[newJob].grades[grade]
